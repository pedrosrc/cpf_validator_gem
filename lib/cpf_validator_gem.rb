    ...
    if valor.length == 11
      unless nulos.member?(valor.join)
        ...
        if resultado1 == valor[9]
          ...
          return { status: true, message: "🟢 O CPF #{cpf} informado é válido." }.to_json if resultado2 == valor[10]
        end
      end
    end
    return { status: false, message: "🔴 O CPF #{cpf} informado é inválido." }.to_json
